...
export async function GET(request: NextRequest) {
  ...
  const entries = await Promise.all(symbols.map(async (symbol) => {
    try {
      const url = `https://api.binance.com/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=15m&limit=49`;
      const response = await fetch(url, { next: { revalidate: 60 } });
      if (!response.ok) return [symbol, []] as const;
      const rows = await response.json() as Kline[];
      const points = rows.map((row) => ({ t: Number(row[0]), c: Number(row[4]) })).filter((p) => Number.isFinite(p.t) && Number.isFinite(p.c));
