...
export default function TradingActivity({ heartbeat, events, trade, error, refreshedAt, enabled, live, onRefresh, onStart, starting }: {
  ...
  return <section className={`panel trading-activity activity-${summary.tone}`} aria-label="Handelsaktivitet">
    ...
    {(!enabled || !live) && onStart && <button type="button" className="primary compact" disabled={starting} onClick={onStart}>Start automatisk handel</button>}
