...
type PortfolioSortKey = "asset" | "type" | "invested" | "value" | "result" | "quantity" | "price" | "leverage" | "status";
type PortfolioSortDirection = "asc" | "desc";
const PORTFOLIO_SORT_COLUMNS: Array<{ key: PortfolioSortKey; label: string }> = [
  { key: "asset", label: "Valuta" },
  { key: "type", label: "Type" },
  { key: "invested", label: "Investert" },
  { key: "value", label: "Verdi nå" },
  { key: "result", label: "Resultat" },
  { key: "quantity", label: "Mengde" },
  { key: "price", label: "Nåpris" },
  { key: "leverage", label: "Giring" },
...
export default function TradingDashboard() {
  ...
  const [portfolioListView, setPortfolioListView] = useState(false);
  const [portfolioSort, setPortfolioSort] = useState<{ key: PortfolioSortKey; direction: PortfolioSortDirection }>({ key: "value", direction: "desc" });
  ...
  const sortedAllocations = useMemo(() => {
    const typeLabel = (allocation: (typeof allocations)[number]) => allocation.derivativeOpen
      ? allocation.derivativeMode === "futures" ? "Futures short" : "Margin short"
      : allocation.owned ? "Spot" : "Ingen aktiv posisjon";
    const statusLabel = (allocation: (typeof allocations)[number]) => allocation.derivativeOpen
      ? "Åpen short"
      : allocation.owned ? "Investert spot" : "Ikke investert";
    const sortValue = (allocation: (typeof allocations)[number], key: PortfolioSortKey): string | number => {
      const result = allocation.derivativeOpen
        ? Number(allocation.derivativeUnrealized ?? 0)
        : Number(allocation.unrealized ?? allocation.pnl ?? 0);
      switch (key) {
        case "asset": return allocation.asset;
        case "type": return typeLabel(allocation);
        case "invested": return allocation.derivativeOpen ? Number(allocation.derivativeEntryValue ?? 0) : Number(allocation.invested ?? 0);
        case "value": return allocation.derivativeOpen ? Number(allocation.derivativeCurrentValue ?? 0) : Number(allocation.currentValue ?? 0);
        case "result": return result;
        case "quantity": return allocation.derivativeOpen ? Number(allocation.derivativeQuantity ?? 0) : Number(allocation.quantity ?? 0);
        case "price": return Number(allocation.currentPrice ?? 0);
        case "leverage": return allocation.derivativeOpen ? Number(allocation.derivativeLeverage ?? 1) : 1;
        case "status": return statusLabel(allocation);
      }
    };
    return [...allocations].sort((a, b) => {
      const aValue = sortValue(a, portfolioSort.key);
      const bValue = sortValue(b, portfolioSort.key);
      const comparison = typeof aValue === "number" && typeof bValue === "number"
        ? aValue - bValue
        : String(aValue).localeCompare(String(bValue), "nb", { numeric: true, sensitivity: "base" });
      return portfolioSort.direction === "asc" ? comparison : -comparison;
    });
  }, [allocations, portfolioSort]);

  function togglePortfolioSort(key: PortfolioSortKey) {
    setPortfolioSort((current) => current.key === key
      ? { key, direction: current.direction === "asc" ? "desc" : "asc" }
      : { key, direction: key === "asset" || key === "type" || key === "status" ? "asc" : "desc" });
  }
  ...
  return <main className="shell">
    ...
    <section className="panel portfolio-panel"><div className="panel-head"><div><p className="eyebrow">PORTEFØLJE</p><h3>Investert per valuta</h3></div><div className="portfolio-view-controls"><span className="muted">Visning</span><label className="portfolio-switch"><span className={!portfolioListView ? "active" : ""}>Kort</span><button type="button" role="switch" aria-checked={portfolioListView} aria-label="Slå listevisning på eller av" className={`switch ${portfolioListView ? "on" : ""}`} onClick={() => setPortfolioListView((value) => !value)}><span /></button><span className={portfolioListView ? "active" : ""}>Liste</span></label></div></div>
      ...
      {!portfolioListView ? <>
        <div className="assets">{allocations.map(({ asset, invested, quantity, pnl, currentPrice, currentValue, unrealized, owned, derivativeOpen, derivativeMode, derivativeQuantity, derivativeLeverage, derivativeEntryValue, derivativeCurrentValue, derivativeUnrealized }) => {
          ...
          const resultValue = allocation.derivativeOpen ? Number(allocation.derivativeUnrealized ?? 0) : Number(allocation.unrealized ?? allocation.pnl ?? 0);
    ...
    <HowItWorks />
    {isCompassInternalAuth && <CommunityPanel />}
  </main>;
}
