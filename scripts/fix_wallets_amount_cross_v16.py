...
# ----- worker: publish Spot + Futures wallet balances -----
patch('trader/worker.py',
...
patch('app/trading-dashboard.tsx',
      ...
      'dashboard total fallback')

# Best setup uses relevant wallet
patch('app/trading-dashboard.tsx',
      ...
      'dashboard suggested amount wallet')

patch('app/trading-dashboard.tsx',
      'settings.leverage, availableCapital]);\n',
      'settings.leverage, availableCapital, futuresAvailable]);\n',
      'dashboard best setup dependencies')

# Prioritize uses editable amount
patch('app/trading-dashboard.tsx',
      ...
      'dashboard prioritize editable amount')

patch('app/trading-dashboard.tsx',
      '      requested_notional: setup.suggested,\n',
      '      requested_notional: requestedAmount,\n',
      'dashboard directive requested amount')
...
# Top summary: show Spot + Futures explicitly
old_top = '''<div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 18, alignItems: "end" }}>\n        <div><span className="label">TOTAL BINANCE-VERDI</span><strong style={{ display: "block", fontSize: "clamp(2.2rem, 5vw, 4.4rem)", lineHeight: 1.05, marginTop: 8 }}>{money(totalAssets)} {settings.quote_asset}</strong><small>Kun faktisk beholdning på Binance, verdsatt til markedspris</small></div>\n        <div><span className="label">Investert på Binance</span><strong style={{ display: "block", fontSize: "1.6rem", marginTop: 8 }}>{money(portfolioValue)} {settings.quote_asset}</strong><small>Faktisk krypto-beholdning nå</small></div>\n        <div><span className="label">Ledig på Binance</span><strong style={{ display: "block", fontSize: "1.6rem", marginTop: 8 }}>{money(availableCapital)} {settings.quote_asset}</strong><small>Faktisk fri {settings.quote_asset}-saldo</small></div>\n      </div>'''
new_top = '''<div className="wallet-summary-grid">\n        <div className="wallet-total"><span className="label">TOTAL BINANCE-VERDI</span><strong>{money(totalAssets)} {settings.quote_asset}</strong><small>Spot + Futures samlet</small></div>\n        <div><span className="label">Spot totalt</span><strong>{money(spotTotal)} {settings.quote_asset}</strong><small>Ledig Spot: {money(spotAvailable)}</small></div>\n        <div><span className="label">Investert Spot</span><strong>{money(portfolioValue)} {settings.quote_asset}</strong><small>Krypto-beholdning i Spot</small></div>\n        <div><span className="label">Futures totalt</span><strong>{money(futuresTotal)} {settings.quote_asset}</strong><small>Ledig Futures: {money(futuresAvailable)}</small></div>\n      </div>'''
patch('app/trading-dashboard.tsx', old_top, new_top, 'dashboard top wallet breakdown')

# Futures isolated -> cross-compatible fallback for -4175
patch('trader/derivatives.py',
      '        except BinanceError as exc:\n            if "-4046" in str(exc):\n                return None\n            raise\n',
      '        except BinanceError as exc:\n            if "-4046" in str(exc) or "-4175" in str(exc):\n                return None\n            raise\n',
      'futures cross-compatible fallback')

# CSS
p = Path('app/globals.css')
s = p.read_text(encoding='utf-8')
css = '''\n\n/* wallet breakdown + editable setup amount v16 */\n.wallet-summary-grid{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:18px;align-items:end}.wallet-summary-grid strong{display:block;font-size:1.55rem;margin-top:8px}.wallet-summary-grid .wallet-total strong{font-size:clamp(2.2rem,5vw,4.4rem);line-height:1.05}.setup-amount{display:grid;gap:5px;margin-top:4px}.setup-amount input{width:100%;box-sizing:border-box;background:rgba(5,15,25,.72);border:1px solid var(--line);border-radius:9px;color:var(--text);padding:8px 10px;font-weight:800}.setup-amount input:focus{outline:2px solid rgba(59,130,246,.35);border-color:rgba(59,130,246,.65)}@media(max-width:900px){.wallet-summary-grid{grid-template-columns:1fr 1fr}}@media(max-width:620px){.wallet-summary-grid{grid-template-columns:1fr}}\n'''
if 'wallet breakdown + editable setup amount v16' not in s:
    p.write_text(s + css, encoding='utf-8')
    print('patched: dashboard css')
else:
    print('already: dashboard css')

print('FIX_WALLETS_AMOUNT_CROSS_V16_OK')
