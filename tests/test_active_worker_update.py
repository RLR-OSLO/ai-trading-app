...
def test_health_requires_both_accounts_new_engine_and_completed_live_cycle():
    rows = [event("owner", "heartbeat", "engine=user-commands-v5;veto_controls=v2;futures_wallet_version=v2;recovery_locked=True"),
            event("second", "heartbeat", "engine=user-commands-v5;veto_controls=v2;futures_wallet_version=v2;recovery_locked=False")]
...
def test_old_heartbeat_or_changed_authorization_cannot_pass_health():
    assert not update.health_ready([event("owner", "heartbeat", "engine=user-commands-v5;recovery_locked=True")], {"owner": False})
    assert not update.health_ready([event("owner", "heartbeat", "engine=exit-guard-v3;recovery_locked=True")], {"owner": False})
    assert not update.health_ready([event("owner", "heartbeat", "engine=user-commands-v5;veto_controls=v2;futures_wallet_version=v2;recovery_locked=False")], {"owner": False})
