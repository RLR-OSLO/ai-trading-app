...
def _macd_positive(values: Sequence[Decimal]) -> bool:
...
def analyze_market(timeframes: dict[str, Sequence[Sequence[object]]]) -> MarketAnalysis:
    ...
    weights = {"15m": 1, "1h": 2, "4h": 2}
    for interval, weight in weights.items():
        values = closes[interval]
        if _ema(values, 8) > _ema(values, 21) and _ema_rising(values, 8) and _ema_rising(values, 21):
            score += weight
            reasons.append(f"{interval}_trend")

    rsi = _rsi(closes["1h"])
    if Decimal("48") <= rsi <= Decimal("68"):
        score += 1
        reasons.append("healthy_rsi")

    if _macd_positive(closes["1h"]):
        score += 1
        reasons.append("positive_macd")
    ...
    return MarketAnalysis(not veto and score >= 7, score, confidence, rsi, atr_percent, volume_ratio, tuple(reasons))


def bullish_btc_regime(timeframes: dict[str, Sequence[Sequence[object]]]) -> bool:
    for interval in ("1h", "4h"):
        closes = _series(timeframes[interval], 4)
        if not (_ema(closes, 8) > _ema(closes, 21) and _ema_rising(closes, 21)):
            return False
    return True
